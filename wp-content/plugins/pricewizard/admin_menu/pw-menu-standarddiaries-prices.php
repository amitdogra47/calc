...
//** edit table name
// Table management = wp_pwpricesstandard

//echo "step1";
//** edit class name and function name and variable name (at end of page)
if( !class_exists('std_diary_prices') ){
	class std_diary_prices{
		function std_diary_prices() { //constructor
...
//** check rounding requirement
					...
					$v3 = round($_POST['sd_diary_var3'], 3); $v4 = $_POST['sd_diary_var4'];
...
//** edit query string
			$updates = $wpdb->update( 'wp_pwpricesstandard', array( 'size' => $v1, 'type' => $v2, 'price' => $v3, 'nice_name' => $v4), array( 'seqid' => $d ) );
			//$wpdb->show_errors();
			//$wpdb->print_error();
			if ($updates === false){
		...
		function updatepage($d) {
			global $wpdb;
			$sd_sql= "SELECT * FROM wp_pwpricesstandard WHERE seqid = $d";
...
?>		
			<div class="wrap">
<!-- edit heading -->
				<h3>Standard diaries - Update price</h3>
				<form id="updatevar" method="post" action="">
					<table>
						...
						<tr valign="top">
							<th scope="row" width="120" style="text-align:left">Style</th>
...
<?php	
		...
		function editpage() {
			...
			<div class="wrap">
<!-- edit heading	-->
			<h3>Standard Diaries - Diary prices (by binding style and size)</h3>
...
<!-- edit table columns as necessary	 -->
				<table class="widefat" style="width:470px;"><tr>
					...
					<th width="120px" align=left>Nice Name</th>
					<th width="80px" align=left>Price</th>
					<th width="40px" align=left>Edit?</th>
				</tr>
				<!-- get_item details -->
<?php 			$editsql= "SELECT * FROM wp_pwpricesstandard ORDER BY seqid";
...
<?php			
	...
	} //END Class std_diary_prices
}  // end if class not exists
if( class_exists('std_diary_prices') )
	$diaryprices = new std_diary_prices();
?>