...
//** edit table name
// Table management = wp_pwqtybreakstd

//echo "step1";
//** edit class name and function name and variable name (at end of page)
if( !class_exists('std_prices_breaks') ){
	class std_prices_breaks{
		function std_prices_breaks() { //constructor
...
//** edit query string
			$updates = $wpdb->update( 'wp_pwqtybreakstd', array( 'breakcode' => $v1, 'breakqty' => $v2), array( 'seqid' => $d ) );
			//$wpdb->show_errors();
			//$wpdb->print_error();
			if ($updates === false){
		...
		function updatepage($d) {
			global $wpdb;
			$sd_sql= "SELECT * FROM wp_pwqtybreakstd WHERE seqid = $d";
...
?>		
			<div class="wrap">
<!-- edit heading -->
				<h3>Standard diaries: Quantity price breaks</h3>
...
<!-- edit table columns as necessary	 -->
				<table class="widefat" style="width:270px;"><tr>
					<th width="30px" align=left>ID</th>
					<th width="100px" align=left>Break Code</th>
					<th width="10px" align=left>Break Qty</th>
					<th width="40px" align=left>Edit?</th>
				</tr>
				<!-- get_item details -->
<?php 			$editsql= "SELECT * FROM wp_pwqtybreakstd ORDER BY seqid";
...
<?php			
	...
	} //END Class std_prices_breaks
}  // end if class not exists
if( class_exists('std_prices_breaks') )
	$diaryprices = new std_prices_breaks();
?>