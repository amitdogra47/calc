...
//** edit table name
// Table management = wp_pwcustompages

//echo "step1";
//** edit class name and function name and variable name (at end of page)
if( !class_exists('std_prices_custompages') ){
	class std_prices_custompages{
		function std_prices_custompages() { //constructor
...
//** check rounding requirement
					...
					$v3 = $_POST['sd_diary_var3'];$v4 = round($_POST['sd_diary_var4'], 3);
					//echo "<br/>step1.3 You clicked Update and you want to change id = ".$id ." to ".$var;
					unset ($_POST['notice']);
					$this->updatetable($id,$v1,$v2,$v3,$v4);
					// then load edit form
					$this->editpage();
				}
		...
		function updatetable($d,$v1, $v2, $v3, $v4){
			global $wpdb;
			//echo "<br/>step3.1 Table update about to take place for id = $d adjusted to var $v1";
//** edit query string
			$updates = $wpdb->update( 'wp_pwcustompages', array( 'breakcode' => $v1, 'customqty' => $v2, 'size' => $v3, 'price' => $v4), array( 'seqid' => $d ) );
			//$wpdb->show_errors();
			//$wpdb->print_error();
			if ($updates === false){
		...
		function updatepage($d) {
			global $wpdb;
			$sd_sql= "SELECT * FROM wp_pwcustompages WHERE seqid = $d";
			//echo "<br/>sql is $sd_sql<br/>";
			$sd_result = $wpdb->get_row($sd_sql,ARRAY_N);
			$t0 = $sd_result[0]; $t1 = $sd_result[1]; $t2 = $sd_result[2]; $t3 = $sd_result[3];$t4 = $sd_result[4];
?>		
			<div class="wrap">
<!-- edit heading -->
				<h3>Standard diaries - Updates prices for custom pages</h3>
				<form id="updatevar" method="post" action="">
					<table>
						...
						<tr valign="top">
							<th scope="row" width="120" style="text-align:left"># of Custom Pages</th>
...
<?php	
		...
		function editpage() {
			...
			<div class="wrap">
<!-- edit heading	-->
			<h3>Standard diaries - Custom pages prices</h3>
...
<!-- edit table columns as necessary	 -->
				<table class="widefat" style="width:470px;"><tr>
					...
					<th width="100px" align=left># of Custom Pages</th>
					<th width="120px" align=left>Size</th>
					<th width="80px" align=left>Price</th>
					<th width="40px" align=left>Edit?</th>
				</tr>
				<!-- get_item details -->
<?php 			$editsql= "SELECT * FROM wp_pwcustompages ORDER BY seqid";
...
<?php			
	...
	} //END Class std_prices_custompages
}  // end if class not exists
if( class_exists('std_prices_custompages') )
	$diaryprices = new std_prices_custompages();
?>