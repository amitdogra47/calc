...
//** edit table name
// Table management = iq_price_stamping = wp_pwstampingcustom

//echo "step1";
//** edit class name and function name and variable name (at end of page)
if( !class_exists('custom_stamping') ){
	class custom_stamping{
		function custom_stamping() {
...
//** check rounding requirement
					$v1 = $_POST['sd_diary_var1']; 
					$v2 = round($_POST['sd_diary_var2'], 3); 
...
//** edit query string
			$updates = $wpdb->update( 'wp_pwstampingcustom', array( 'breakcode' => $v1, 'price' => $v2 ), array( 'seqid' => $d ) );
			//$wpdb->show_errors();
			//$wpdb->print_error();
			if ($updates === false){
		...
		function updatepage($d) {
			global $wpdb;
			$sd_sql= "SELECT * FROM wp_pwstampingcustom WHERE seqid = $d";
...
?>		
			<div class="wrap">
<!-- edit heading -->
				<h3>Custom diaries - stamping price & stamping qty break</h3>
...
<?php	
		...
		function editpage() {
			...
			<div class="wrap">
<!-- edit heading	-->
			<h3>Custom diaries - Stamping prices (by quantity break)</h2>
...
<!-- edit table columns as necessary	 -->
				<table class="widefat" style="width:300px;"><tr>
					...
					<th width="100px" align=left>BreakCode</th>
					<th width="200px" align=left>Price</th>
					<th width="100px" align=left>Edit?</th>
				</tr>
				<!-- get_item details -->
<!-- edit sql string -->
<?php 			$editsql= "SELECT * FROM wp_pwstampingcustom ORDER BY breakcode";
...
<?php			
	...
	} //END Class custom_stamping
}  // end if class not exists
if( class_exists('custom_stamping') )
$stamping = new custom_stamping();
?>