...
//** edit table name
// Table management = wp_pwoptionscovers

//echo "step1";
//** edit class name and function name and variable name (at end of page)
if( !class_exists('std_prices_coveroptions') ){
	class std_prices_coveroptions{
		function std_prices_coveroptions() { //constructor
...
//** edit query string
			$updates = $wpdb->update( 'wp_pwoptionscovers', array( 'cover_type' => $v1, 'cover_option' => $v2,'breakcode' => $v3, 'size' => $v4, 'price' => $v5), array( 'seqid' => $d ) );
			//$wpdb->show_errors();
			//$wpdb->print_error();
			if ($updates === false){
		...
		function updatepage($d) {
			global $wpdb;
			$sd_sql= "SELECT * FROM wp_pwoptionscovers WHERE seqid = $d";
...
?>		
			<div class="wrap">
<!-- edit heading -->
				<h3>Cover options: prices by break and size</h3>
				<form id="updatevar" method="post" action="">
					<table>
						...
						<tr valign="top">
							<th scope="row" width="120" style="text-align:left">Cover Option</th>
...
<!-- edit table columns as necessary	 -->
				<table class="widefat" style="width:500px;"><tr>
					<th width="30px" align=left>ID</th>
					<th width="100px" align=left>Cover Type</th>
					<th width="100px" align=left>Cover Option</th>
				...
				<!-- get_item details -->
<?php 			$editsql= "SELECT * FROM wp_pwoptionscovers ORDER BY seqid";
...
<?php			
	...
	} //END Class std_prices_coveroptions
}  // end if class not exists
if( class_exists('std_prices_coveroptions') )
	$diaryprices = new std_prices_coveroptions();
?>